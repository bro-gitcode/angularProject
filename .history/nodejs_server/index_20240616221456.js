...
app.post('/project',(req,res)=>{
    ...
    db.query(sql,[name_project,create_date,price,leader,member.join(',')],(err,data)=>{
...
app.delete('/project/:id',(req,res)=>{
    ...
    db.query(sql,id,(err,d)=>{
        if(err) res.json({'message':"Can't delete !" + err});
...
app.post('/staff',(req,res)=>{
    let {first_name,last_name,day_of_birth,sect,area} = req.body;
    let sql = "INSERT INTO staff SET first_name=?,last_name=?,day_of_birth=?,sect=?,area=?";
    db.query(sql,[first_name,last_name,day_of_birth,sect,area],(err,data)=>{
        if(err) res.json({'message':"Can't insert into" + err});
        else res.json({'message':'Ok insert into'});
    });
});
...
app.put('/staff/:id',(req,res)=>{
    ...
    let id = req.params.id;
    let {first_name,last_name,day_of_birth,sect,area} = req.body;
    let sql = "UPDATE staff SET first_name=?,last_name=?,day_of_birth=?,sect=?,area=? WHERE id=?";
    db.query(sql,[first_name,last_name,day_of_birth,sect,area,id],(err,d)=>{
        if(err) res.json({'message':"Can't update !" + err});
        else res.json({'message':'Updated !'});
    })
})
...
app.post('/task',(req,res)=>{
    let {name_task,id_project,id_staff,description,status,priority} = req.body;
    let sql = "INSERT INTO task SET name_task=?,id_project=?,id_staff=?,description=?,status=?,priority=?";
    db.query(sql,[name_task,id_project,id_staff,description,status,priority],(err,data)=>{
        if(err) res.json({'message':"Can't insert into" + err});
        else res.json({'message':'Ok insert into'});
    });
});
...
app.put('/task/:id',(req,res)=>{
    ...
    let id = req.params.id;
    let {first_name,last_name,day_of_birth,sect,area} = req.body;
    let sql = "UPDATE task SET name_task=?,id_project=?,id_staff=?,description=?,status=?,priority=? WHERE id=?";
    db.query(sql,[name_task,id_project,id_staff,description,status,priority,id],(err,d)=>{
        if(err) res.json({'message':"Can't update !" + err});
        else res.json({'message':'Updated !'});
    })
})
