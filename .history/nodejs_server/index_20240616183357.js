...
app.get('/staff',(req,res,next)=>{
    let sql = `SELECT id,first_name,last_name,day_of_birth,sect,area FROM staff ORDER BY id desc`;
    db.query(sql,(err,data)=>{
        if(err) res.json({'message': err});
        else res.json(data);
    });
...
app.get('/staff/:id',(req,res,next)=>{
    ...
    let sql = `SELECT id,first_name,last_name,day_of_birth,sect,area FROM staff WHERE id=? ORDER BY id desc`;
    db.query(sql,id,(err,data)=>{
        if(err) res.json({'message': err});
...
app.get('/task',(req,res,next)=>{
    let sql = `SELECT id,name_task,id_project,id_staff,description,status,priority FROM task ORDER BY id desc`;
    db.query(sql,(err,data)=>{
        if(err) res.json({'message': err});
        else res.json(data);
    });
...
app.get('/task/:id',(req,res,next)=>{
    ...
    let sql = `SELECT id,name_task,id_project,id_staff,description,status,priority FROM staff WHERE id=? ORDER BY id desc`;
    db.query(sql,id,(err,data)=>{
        if(err) res.json({'message': err});
        else if (data.lenngth == 0) res.json({'message': 'No staff'})
        else res.json(data[0]);
